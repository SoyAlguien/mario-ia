...
function game(){
  ...
  if(gameConfig.status==='start'){
    ...
    textSize(40);
    textAlign(CENTER);
    text("Presiona el boton \"JUGAR\" para comenzar ", gameConfig.screenX/2, gameConfig.screenY/2);
    textSize(40);
...
function changeGameStatud(character){
  if((keyDown(control.up) ||JUGAR||keyDown(control.left)||keyDown(control.right) )&& gameConfig.status==="start") {
